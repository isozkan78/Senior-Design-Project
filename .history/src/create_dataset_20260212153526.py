...
try:
    # Proje ismini buraya sabitledik
...
# .toInt() ekledik: Ondalıklı sayıları tam sayıya çevirir, hatayı önler.
dataset = fire_collection.map(lambda img: img.select(0).toInt().reduceToVectors(
    geometry=roi, scale=1000, geometryType='centroid', labelProperty='label'
)).flatten()
# -------------------------------
...
task = ee.batch.Export.table.toDrive(
    ...
    description='Antalya_Yangin_Verisi_Final', # İsim yine değişti karışmasın diye
    fileFormat='CSV',
