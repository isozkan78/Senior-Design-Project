...
# 1. GEE Başlatma (Senin proje isminle)
print("🔄 Google Earth Engine bağlantısı kontrol ediliyor...")
try:
    # Proje ismini buraya senin için sabitledim
...
# Görüntünün sadece ilk bandını (select(0)) seçiyoruz ki hata vermesin.
dataset = fire_collection.map(lambda img: img.select(0).reduceToVectors(
...
task = ee.batch.Export.table.toDrive(
    ...
    description='Antalya_Yangin_Verisi_Fixed', # Dosya adı karışmasın diye değiştirdim
    fileFormat='CSV',
...
print("Task Manager'ı tekrar kontrol et: https://code.earthengine.google.com/tasks")